  ...
  return {
    init: function(path, onready) {
      ...
      if (jsonrpc_interface.indexOf("http") == 0) {
        ...
        jsonrpc_ws.onerror = function(event) {
          ...
          main_alert("alert-error", "websocket 错误. 你可能需要刷新本页面.");
          ws_callback = {};
        };
    ...
    tell_active: function(keys) {
      ...
      ARIA2.request("tellActive", keys,
        function(result) {
          ...
          if (select_lock) return;
          if (!result.result) {
            main_alert("alert-error", "<strong>错误: </strong>rpc 返回错误.", 5000);
          }
          ...
          if (finished_tasks_list === undefined) {
            ...
            $.each(result, function(i, e) {
              ...
              if (ARIA2.finish_notification) {
                YAAW.notification("下载任务完成", e.title);
              }
              finished_tasks_list.push(e.gid);
            });
          }
    ...
    change_pos: function(gid, pos, how) {
      ARIA2.request("changePosition", [gid, pos, how],
        function(result) {
          ...
          main_alert("alert-info", "已改变下载顺序", 1000);
          ARIA2.refresh();
        }
      );
    },
    ...
    unpause: function(gids) {
      ...
      ARIA2.batch_request("unpause", gids,
        function(result) {
          ...
          if (error.length == 0) {
            main_alert("alert-info", "已启动", 1000);
            ARIA2.refresh();
          } else {
            main_alert("alert-error", error.join("<br />"), 3000);
          }
    ...
    remove: function(gids) {
      ...
      ARIA2.batch_request("remove", gids,
        function(result) {
          ...
          if (error.length == 0) {
            main_alert("alert-info", "已移除", 1000);
            ARIA2.refresh();
          } else {
            main_alert("alert-error", error.join("<br />"), 3000);
          }
    ...
    change_options: function(gid, options) {
      ARIA2.request("changeOption", [gid, options],
        function(result) {
          ...
          main_alert("alert-info", "设置已更新", 1000);
        }
      );
    },
    ...
    pause_all: function() {
      ARIA2.request("pauseAll", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "已暂停所有任务. 请等待tracker服务器响应.", 2000);
        }
      );
    },
    ...
    purge_download_result: function() {
      ARIA2.request("purgeDownloadResult", [],
        function(result) {
          ...
          ARIA2.refresh();
          main_alert("alert-info", "已清除所有 完成/错误/回收站 任务.", 2000);
        }
      );
    },
      ...
      ARIA2.request("changeOption", [gid, options],
        function(result) {
          if (!result.result) {
            ...
            main_alert("alert-success", "设置更改成功!", 2000);
          }
        }
      );
    },
    ...
    auto_refresh: function(interval) {
      ...
      interval_id = window.setInterval(function() {
        ...
        if (select_lock) {
          if (need_refresh) {
            main_alert("", "任务列表已有改变,请点击'刷新'更新任务列表.");
          }
        } else {
          if (need_refresh) {
            ARIA2.refresh();
